...
class retinaface(IMDB):
    def __init__(self, image_set, root_path, data_path):
        ...
        for line in open(label_file, 'r'):
            ...
            file_name = file.replace("widerFace/WIDER_train/images/", "")
            count = int(line[1])
            lables = [item for item in line[2:]]
            all_lables = [lables[i:i + 5] for i in range(0, count, 5)]
            all_lables = [" ".join(l) for l in all_lables]
            self._fp_bbox_map[file_name] = all_lables
            # if line.startswith('#'):
            #     name = line[1:].strip()
