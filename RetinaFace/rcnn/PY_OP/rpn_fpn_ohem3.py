...
class RPNFPNOHEM3Operator(mx.operator.CustomOp):
    ...
    def forward(self, is_train, req, in_data, out_data, aux):
        ...
        # print("===========>>>>forward ", cls_score.shape, labels_raw.shape)
...
class RPNFPNOHEM3Prop(mx.operator.CustomOpProp):
    ...
    def infer_shape(self, in_shape):
        ...
        ret = in_shape, [labels_shape, anchor_weight_shape, [labels_shape[0], 1]]
        # print(ret)
        return ret
...
if __name__ == '__main__':
    ...
    dataset = "retinaface"
    prefix = "face"
    rpn_cls_score_reshape = mx.nd.ones((1, 2, 3))
    rpn_cls_score_reshape = mx.symbol.Variable(name="rpn_cls_score_reshape")
    label = mx.nd.ones((1, 3))
    label = mx.symbol.Variable(name="label")

    label, anchor_weight, valid_count = mx.sym.Custom(op_type='rpn_fpn_ohem3', stride=int(stride), network=network, dataset=dataset, prefix=prefix,
                                                      cls_score=rpn_cls_score_reshape, labels=label)
    # print(label)
    # print(anchor_weight)
    # print(valid_count)
    #
    # ret = label.infer_shape(rpn_cls_score_reshape=(1, 2, 3), label=(1, 3))
    # print(ret)
    e = anchor_weight.bind(mx.cpu(), {"rpn_cls_score_reshape": mx.nd.ones((1, 2, 800)), "label": mx.nd.ones((1, 800))})
    y = e.forward()
    print(y)
