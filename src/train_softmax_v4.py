...
def parse_args():
    ...
    parser.add_argument('--pretrained', default='./train/models_2019-11-18-18:45:00/model,650', help='pretrained model to load')
...
def train_net(args):
    ...
    if len(args.pretrained) == 0:
        ...
        conv0_weight = arg_params['conv0_weight'].asnumpy()
        logging.info("conv0_weight %s %s", conv0_weight.max(), conv0_weight.min())
        # sys.exit()
        sym, arg_params, aux_params = get_symbol(args, arg_params, aux_params)
